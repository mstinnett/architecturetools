...
**Family accents — SHIPPED.** Canonical tokens live in `tokens.css`; visual reference in `palette-shipped.html`. The system: family tier **anchored to Prussian's own lightness (OKLCH L 0.41)** with per-hue chroma — a serious, rich, high-contrast (AAA on paper) set rather than a light muted one. Two documented exceptions: **Ink** (achromatic anchor) and **ochre/building** (lifted to L 0.55, because yellow can't be both dark and vivid in sRGB — a gamut fact, not a tuning choice). Hover/tint/dark-mode variants are *derived* (hover = L−0.06; tint = L 0.94 low-chroma; dark = lifted L), so the whole UI ramps from these anchors.
