...
- [x] **Partition "solve for the boundary" mode (operator workflow), A1–A3.** Source: operator ask 2026-06-08; stories in `docs/partition-stories.md`. Added a forward⇄solve direction toggle to `partition.html`. Solve has two sub-modes: **Fit a run** (nearest whole-tile runs above/below a target + the move to each, half-tile *sliver* flagged) and **Place a seam** (a transition inside a fixed total — left zone whole, with the insight that the right end cut `(total + 2·joint) mod module` is invariant of where the seam sits; zero ⇒ both sides whole anywhere). Exact lattice search, reuses the format helpers + figure. Verified end-to-end (fit/sliver/metric, seam both-whole/unavoidable-cut/seam-near/too-short). Remaining story groups (A4 multi-zone, B size/joint, C anchor, D two-grid, E ordering, F tolerance) stay in `docs/partition-stories.md` as future work.
- [x] **Partition tool page (convert.html-style) on `partition.js`.** Source: HANDOFF.md §5. `calculators/partition.html` — the UI layer over the primitive. Parses a run with `parse-length`, runs `partition` in the chosen mode (tiles / sections / on-center / balusters), and draws the layout + residual as an SVG strip (run dimension on top, parts in the strip, residual hatched — the same "the figure is the product" idiom as `convert.html`'s number line). Reuses `numberline`'s format helpers; overall lengths read as feet-inches, per-item modules as plain inches ("14 1/2" o.c."). Carries the clearer Min/Max-style copy from the review (spacing/gap ≤ max ✓ checks). Infeasible layouts render copy from the primitive's stable `reason` code. `noindex` added. Gate green; verified end-to-end against the four worked examples + metric + over-`MAX_DRAW` (summary fallback) cases.
