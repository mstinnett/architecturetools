...
- [ ] **Bring the remaining pages onto the shared stylesheets.** Source: PROJECT.md. The picker (`index.html`) and converter (`convert.html`) now share `assets/css/global2.css` + `assets/css/tokens.css` (family accents via `data-family`, one dark scheme — see Done below). The legacy calculators, `components.html`, and `site-screen.html` still carry their own styles or the old `global.css`. Migrate as pages are touched.
